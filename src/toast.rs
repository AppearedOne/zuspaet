...
impl Toast {
    pub fn new(title: &str, body: &str, status: Status) -> Self {
        Toast {
            title: title.to_string(),
            body: body.to_string(),
            status,
            time: chrono::Local::now().naive_local().time(),
        }
    }
}
    ...
    Message: 'a + Clone,
{
    pub fn new(
        ...
        let toasts = toasts
            ...
            .map(|(index, toast)| {
                container(column![
                    container(row![horizontal_space().height(1)].align_y(Center))
                        .width(Fill)
                        .padding(2)
                    ...
                    container(column![
                        ...
                        text(toast.body.as_str()).size(11)
                    ])
                    .width(Fill)
                    .padding(5)
